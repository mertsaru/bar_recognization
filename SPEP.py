...
line_names = {0: "albumin", 1: "alpha1", 2: "alpha2", 3: "beta", 4: "gamma"}
...
class spep:
    ...
    def read(self, edge_multiplier=500, amplifier=1.3):
        ...
        edge_img_rgb = Image.fromarray(
            (edge_mx * edge_multiplier).astype(np.uint8), "RGB"
        )
        edge_img_gray = edge_img_rgb.convert(
            "L"
        )  # convert library indicates we can transform while in matrix form. Maybe it would be faster? We can interfere in edge_mx *  multiplier.astype(uint8)
        ...
        self.threshold = (
            np.median(img_col_mean) * amplifier
        )  # increasing threshold for eliminating black-space better
    ...
    def albumin_reference(
        ...
        img_cutted_col_mx = self.img_subject[
            :, left_col_index:right_col_index
        ]  # Cutting left and right sides to find top of the albumin better
        ...
        return self.img_color[
            top_row_index:bottom_row_index, left_col_index:right_col_index, :
        ]

    ## Finding bar lenght
    @classmethod
    def barLenght_reference(
        cls,
        self,
        step=3,
        search_width=[0, 0.25],
        search_lenght=[0, 0.25],
        min_bar_height=10,
    ):
        ...
        img_mean_row_right = img_cut[:, round(self.cutout["width"] / 2) : -1].mean(
            axis=1
        )
        ...
        while (i < self.img_height) and (not (switch)):
            left_row = img_mean_row_left[
                self.img_height - 1 - i
            ]  # bottom i^th row's left half
            right_row = img_mean_row_right[
                self.img_height - 1 - i
            ]  # bottom i^th row's right half
            if (left_row > self.threshold) and (right_row > self.threshold):
                switch = True
                for j in range(1, min_bar_height):
                    left_row_test = img_mean_row_left[
                        self.img_height - 1 - i - j
                    ]  # j above row of row_b
                    right_row_test = img_mean_row_right[self.img_height - 1 - i - j]
                    if (left_row_test < self.threshold) or (
                        right_row_test < self.threshold
                    ):
                        skip_step = j
                        i += skip_step + 1
                        switch = False
        ...
        self.test_color = self.img_color[
            self.test["top"] : self.test["top"] + self.bar,
            self.test["left"] : self.test["right"],
            :,
        ]
    ...
    def locate_test(self, step=3, search_width=[0, 0.25], search_lenght=[0, 0.25]):
        ...
        if (self.img_height - self.test["top"]) < (self.bar):
            ...
            buffer_zone_color = np.array(
                [
                    [[255 for rgb in range(3)] for row in range(difference)]
        ...
        self.test_orj = self.img_orj[
            self.test["top"] : self.test["bottom"],
            self.test["left"] : self.test["right"],
        ]
        ...
        self.test_subject = self.img_subject[
            self.test["top"] : self.test["bottom"],
            self.test["left"] : self.test["right"],
        ]
    ...
    def find_bar_dist(self, left_bar, search_width):  # Uses mask to find bars
        ...
        if starting_point >= 0:
            ...
            while i < min(ending_point, self.img_width - self.cutout["width"]):
                cutout = self.img_subject[
                    self.test["top"] : self.test["top"] + self.bar,
                    i : i + self.cutout["width"],
                ]
    ...
    def locate_gamma(cls, self, search_width=1):
        ...
        self.gamma = {"left": left, "right": right}
    ...
    def locate_alpha(cls, self, search_width=1):
        ...
        right = left + self.cutout["width"]
        self.alpha = {"left": left, "right": right}
    ...
    def locate_kappa(cls, self, search_width=1):
        ...
        right = left + self.cutout["width"]
        self.kappa = {"left": left, "right": right}
    ...
    def locate_lambda(cls, self, search_width=1):
        ...
        right = left + self.cutout["width"]
        self.Lambda = {"left": left, "right": right}
    ...
    def locate_lru(self, step=3, search_width=[0, 0.25], search_lenght=[0, 0.25]):
        ...
        while i < width_cut:
            ...
            while j < column_cut:
                cutout = self.img_subject[
                    i : i + self.cutout["height"], j : j + self.cutout["width"]
                ]
                folder_value = cutout.sum()
                if folder_value > folder_max:
                    folder_max = folder_value
    ...
    def locate_bars(self):
        ...
        self.gamma_orj = self.img_orj[
            self.test["top"] : self.test["bottom"],
            self.gamma["left"] : self.gamma["right"],
        ]
        self.gamma_color = self.img_color[
            self.test["top"] : self.test["bottom"],
            self.gamma["left"] : self.gamma["right"],
            :,
        ]

        self.alpha_orj = self.img_orj[
            self.test["top"] : self.test["bottom"],
            self.alpha["left"] : self.alpha["right"],
        ]
        self.alpha_color = self.img_color[
            self.test["top"] : self.test["bottom"],
            self.alpha["left"] : self.alpha["right"],
            :,
        ]

        self.mu_orj = self.img_orj[
            self.test["top"] : self.test["bottom"], self.mu["left"] : self.mu["right"]
        ]
        self.mu_color = self.img_color[
            self.test["top"] : self.test["bottom"],
            self.mu["left"] : self.mu["right"],
            :,
        ]

        self.kappa_orj = self.img_orj[
            self.test["top"] : self.test["bottom"],
            self.kappa["left"] : self.kappa["right"],
        ]
        self.kappa_color = self.img_color[
            self.test["top"] : self.test["bottom"],
            self.kappa["left"] : self.kappa["right"],
            :,
        ]

        self.lambda_orj = self.img_orj[
            self.test["top"] : self.test["bottom"],
            self.Lambda["left"] : self.Lambda["right"],
        ]
        self.lambda_color = self.img_color[
            self.test["top"] : self.test["bottom"],
            self.Lambda["left"] : self.Lambda["right"],
            :,
        ]
    ...
    def graph_creator(self, bar, range=5):
        ...
        bar_value = bar_value[
            :, self.cutout["mid"] - range : self.cutout["mid"] + range
        ]
    ...
    def find_peak_sharp_v3(self):
        ...
        while i < len(self.delta_row) - 1:
            if (
                (self.delta_row[i] > 0)
                and (self.delta_row[i] < 1)
                and (self.delta_row[i + 1] > -1)
                and (self.delta_row[i + 1] < 0)
            ):
                self.lines.append(i)
                i += 1
            else:
            ...
            if (self.delta_row[i] > 0) and (self.delta_row[i] < 1):  # sign i >= 0
                ...
                for k in range(i + 1, i + smoothRange + 1):
                    if self.delta_row[k] >= 0:  # if sign i+smoothRange and i are same
                        ...
                        if k == i + 1:
                            ...
                            i = (
                                k + smoothRange
                            )  # start from k+smoothRange since we know k is positive but k-1 is negative, and Back-loop will fail until k
                        break
                ...
                for j in range(1, smoothRange + 1):
                    if self.delta_row[i - j] < 0:  # if sign i-j and i are not same
                        ...
                        i = (
                            i - j + smoothRange + 1
                        )  # start from there since until that point everything will fail with back-loop because of i-jth point
                        break
                ...
                i = (
                    i + smoothRange + 1
                )  # If we passed until here, we know i+1 until i+smoothRange are all negative so they will fail anyways
            else:
                i += 1
        ...
        if use_combinations:
            ...
            for combination in combinations:
                self.line_method(
                    ...
                    dentRange=dentRange,
                    combine_lines=combine_lines,
                    increase_spec=increase_spec,
                )
                if len(self.lines) == 5:
                    break
